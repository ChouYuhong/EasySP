...
if __name__ == '__main__':
    ...
    # inv_dt = dt + torch.log(-torch.expm1(-dt))
    # dt_bias = inv_dt
    # alpha = torch.randn(num_heads)
    # g = mamba_decay(A_log, alpha, dt_bias)
    
    x = torch.randn(1, 256, 1024).to(torch.bfloat16).cuda()
    gla_layer = DecayLinearAttention().to(torch.bfloat16).cuda()
    o = gla_layer(x)
    import pdb
    pdb.set_trace()